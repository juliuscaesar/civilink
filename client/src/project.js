...
class Project extends React.Component {
    constructor(props) {
         ...
         */
        this.state = {errorMessage: '', displayError: false, project: [], tasks: [], community: [],
            organizer: []};
    ...
    parseInfoResponse(error, response) {
        switch(response.status) {
            case 200:
                this.setState({project: response.body.project, tasks: response.body.tasks,
                    community: response.body.project.community, organizer: response.body.project.user});
                break;
            case 203:
                this.setState({
           ...
           <div>
             ...
             <div className="row">
               <div className="col-xs-8 col-sm-9">
                 <h4>{this.state.tasks[i].title}</h4>
               </div>
               <div className="col-xs-4 col-sm-3">
                 <a href="" className="btn btn-success">Assign me</a>
               </div>
             </div>
             <div className="row">
               <div className="col-xs-12">
                 <p>{this.state.tasks[i].desc}</p>
                 <p>Users assigned: {this.state.tasks[i].assigned.length}/{this.state.tasks[i].needed}
                 · Points offered: {this.state.tasks[i].points}</p>
               </div>
             </div>
           </div>
       )
                   ...
                   <div className="col-sm-10">
                     <div className="row">
                       ...
                       <div className="col-sm-8 col-md-8">
                         <div className="content-box">
                            <p className="headertext">{this.state.project.title}</p>
                            <hr></hr>
                            <p>{this.state.project.desc}</p>
                            <p>Organizer: <a href={this.getProfileUrl()}>{this.state.organizer.firstName} {this.state.organizer.lastName}</a></p>
                         </div>
